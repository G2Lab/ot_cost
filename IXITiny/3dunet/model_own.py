...
import sys
sys.path.append(f'{ROOT_DIR}/code/IXITiny/3dunet/')
...
class DoubleConv(nn.Sequential):
    ...
    def __init__(self, conv1_in_channels, conv1_out_channels, conv2_in_channels, conv2_out_channels, kernel_size=3, order='gcr', num_groups=8, padding=1,
                 is3d=True):
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, apply_pooling=True,
                 pool_kernel_size=2, pool_type='max', basic_module=SingleConv, conv_layer_order='gcr',
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv_kernel_size=3, scale_factor=(2, 2, 2), basic_module=SingleConv,
                 conv_layer_order='gcr', num_groups=8, mode='nearest', padding=1, is3d=True):
...
class UNet3D(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.encoders = nn.ModuleList([
            Encoder(in_channels, 16, 16, 32),
            Encoder(32, 32,32, 64),
            Encoder(64, 64, 64, 128),
            Encoder(128, 128, 128, 256)
        ])
        self.bridge = SingleConv(256, 384)
        # Decoder layers
        self.decoders = nn.ModuleList([
            Decoder(384, 128,128, 128),
            Decoder(128, 64, 64, 64),
            Decoder(64, 32, 32,32)
        ])
